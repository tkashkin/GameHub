			...
			foreach(var emu in emulators)
			{
				...
				if(pattern.length > 0)
				{
					...
					foreach(var sp in subpatterns)
					{
						...
						var files_list = Utils.exec({"find", root.get_path(), "-path", "*/" + sp, "-type", "f"}).log(false).sync(true).output;
				...
				try
				{
					...
					if(supported_extensions != null && supported_extensions.length > 0)
					{
						...
						foreach(var ext in exts)
						{
							...
							var files_list = Utils.exec({"find", root.get_path(), "-path", "*/*." + ext, "-type", "f"}).log(false).sync(true).output;
							var files = files_list.split("\n");
			...
			private File? find_by_pattern(string? pattern, File directory, string basename)
			{
				if(pattern != null)
				{
					...
					foreach(var sp in subpatterns)
					{
						...
						var files_list = Utils.exec({"find", directory.get_path(), "-path", "*/" + sp}).log(false).sync(true).output;
						var files = files_list.split("\n");
